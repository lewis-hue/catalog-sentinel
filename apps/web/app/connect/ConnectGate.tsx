...
export function ConnectGate({ viewerOrigins }: { viewerOrigins: string[] }) {
  ...
  const ready = status?.status === 'READY' && status.loginMode === 'steel' && status.liveLoginAvailable === true;
  if (!ready) {
    return (
      <div className="card" role="alert" style={{ maxWidth: 680, borderColor: 'var(--wrong-edge)' }}>
        ...
        <h2 style={{ margin: '0 0 8px' }}>Steel is not ready</h2>
        <p className="rail-sub" style={{ marginTop: 0 }}>
          For your protection, DistroKid sign-in cannot start unless the API reports an available Steel session.
          No alternate browser provider will be used.
        </p>
        <p className="hint">
          {error || 'The required Steel readiness checks did not pass.'}
        </p>
  ...
  return (
    <>
      <div className="notice-banner" role="status" style={{ borderColor: 'var(--live-edge)', background: 'var(--live-tint)', color: 'var(--live)' }}>
        <strong>Steel ready.</strong>&nbsp; Your attended DistroKid sign-in will run in an isolated Steel browser session.
      </div>
